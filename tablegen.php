<?php
$db="test";//configure your database name
$con = mysqli_connect("localhost","root","","");
 $select= "select distinct table_name from information_schema.columns where table_schema = '$db' order by table_name,ordinal_position";
  //table name to be fetch into information_schema
  $run = mysqli_query($con,$select);
  $table=array();
  $tcnt=0;
  while($row=mysqli_fetch_array($run))
  {
	  $table[$tcnt++]=$row['table_name'];//store table name $table variable
  }
  $column[][]=array();
  $tcnt=0;
  foreach($table as $t)
  {
	...
	$select= "select table_name,column_name,data_type from information_schema.columns where table_schema = '$db' and table_name = '$t' order by table_name,ordinal_position";
		//get perticular table name field tobe get.
	$run = mysqli_query($con,$select);
	 echo "<pre>";
	 ...
	 while($row=mysqli_fetch_array($run))
	 {
		  ...
		  //table name with its colum name to be store in to two dimentinal array
	 }
	 $tcnt++; 
  ...
  print_r($column);
	...
	fwrite($fp,"\t{\n");
	fwrite($fp,"\t\treturn \$this->fields[self::$".$column[$i][$j]."];\n");
	fwrite($fp, "\t}\n");
	
	}
	fwrite($fp,"\tfunction setMultipledata(\$row)\n\t{\n");
	for($j=1;$j<count($column[$i]);$j++)
	{
	$fcname=$column[$i][$j];
	$cname="";	
	$n=$j-1;
	fwrite($fp,"\t\t\$this->fieldsName[self::$".$column[$i][$j]."] = \"".$column[$i][$j]."\";\n");
	fwrite($fp,"\t\t\$this->fieldsType[self::$".$column[$i][$j]."] = Fields::\$text;\n");
	fwrite($fp, "\t\t\$this->fields[self::$".$column[$i][$j]."] = \$row[".$n."];\n");
	
	}
	fwrite($fp, "\n\t}\n");
	fwrite($fp,"\tfunction fetchId(\$data)\n\t{\n ");
	fwrite($fp,"\tparent::setWhere('id='.\$data); ");
	fwrite($fp,"\n\t\$query =parent::preparedExecuteNoneQuery(Common::\$select);
		\t\$run =parent::executeScalar(\$query);	
	...
	fwrite($fp,"\t\t\t\$this->fieldsName[self::$".$column[$i][$j]."] = \"".$column[$i][$j]."\";\n");
	fwrite($fp,"\t\t\t\$this->fieldsType[self::$".$column[$i][$j]."] = Fields::\$text;\n");
	fwrite($fp, "\t\t\t\$this->fields[self::$".$column[$i][$j]."] = \$row['".$column[$i][$j]."'];\n");	
	}
	fwrite($fp, "\t\t}\n\t}\n");
	fwrite($fp,"}\n");
	fwrite($fp,"?>\n");
